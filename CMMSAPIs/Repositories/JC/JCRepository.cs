    ...
    public class JCRepository : GenericRepository
    {
        ...
        internal async Task<List<CMJCDetail>> GetJCDetail(int jc_id)
        {
            ...
            */
            ...
            string myQuery6 = $" SELECT CONCAT(user.firstName,' ',user.lastName) as name,ptwEmpList.employeeId as id ,usd.designationName as responsibility FROM permitemployeelists as ptwEmpList  LEFT JOIN users as user ON user.id = ptwEmpList.employeeId left join userdesignation as usd on usd.id=user.designation_id  where ptwEmpList.JC_id ={jc_id} and ptwEmpList.status=1";
