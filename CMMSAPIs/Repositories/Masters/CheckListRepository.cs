...
namespace CMMSAPIs.Repositories.Masters
{
    public class CheckListRepository : GenericRepository
    {
        ...
        #region CheckPoint
        internal async Task<List<CMCheckPointList>> GetCheckPointList(int checklist_id,  string facilitytimeZone)
        {
            /*
            ...
            if (valid == 1)
            {
                ...
                if (dtCheckpoints != null && m_errorLog.GetErrorCount() == 0)
                {
                    ...
                    foreach (DataRow row in dtCheckpoints.Rows)
                    {
                        ...
                        checkpoint_type.min = (int)Convert.ToDouble(row["range_min"]);
                        checkpoint_type.max =(int) Convert.ToDouble(row["range_max"]);
