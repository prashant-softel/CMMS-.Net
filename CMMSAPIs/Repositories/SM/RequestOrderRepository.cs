...
namespace CMMSAPIs.Repositories.SM
{
    public class RequestOrderRepository : GenericRepository
    {
        ...
        internal async Task<CMDefaultResponse> CreateRequestOrder(CMRequestOrder request, int userID)
        {
            ...
            {
                string poInsertQuery = $" INSERT INTO smrequestorder (plantID,vendorID,receiverID,generated_by,requestDate,status," +
                    ...
                    $"VALUES({request.plantID},{request.vendorID}, {request.receiverID}, {userID}, '{DateTime.Now.ToString("yyyy-MM-dd")}', {request.status}," +
            ...
            CMDefaultResponse response = new CMDefaultResponse(1, CMMS.RETRUNSTATUS.SUCCESS, "Request order created successfully.");
            return response;
        }
        internal async Task<CMDefaultResponse> UpdateGO(CMRequestOrder request, int userID)
        {
            ...
            for (var i = 0; i < request.go_items.Count; i++)
            {
                string updateQ = $"UPDATE smrequestorderdetails SET assetItemID = {request.go_items[i].assetItemID},cost = {request.go_items[i].cost},ordered_qty = {request.go_items[i].ordered_qty},location_ID = {request.location_ID}" +
                    ...
                    $"challan_date = '{request.challan_date.Value.ToString("yyyy-MM-dd")}',requestdate='{request.requestdate.Value.ToString("yyyy-MM-dd")}', " +
        ...
        internal async Task<List<CMRequestOrder>> GetRequestOrderList(int plantID, DateTime fromDate, DateTime toDate)
        {
            ...
            string filter = " (DATE(po.requestDate) >= '" + fromDate.ToString("yyyy-MM-dd") + "'  and DATE(po.requestDate) <= '" + toDate.ToString("yyyy-MM-dd") + "')";
            ...
            string stmt = "SELECT pod.ID as podID,pod.spare_status,pod.remarks,sai.orderflag,sam.asset_name,sam.asset_type_ID,pod.requestID," +
                "pod.assetItemID,sai.serial_number,sai.location_ID,pod.cost ,pod.ordered_qty,po.remarks as rejectedRemark,po.plantID,po.requestDate," +
