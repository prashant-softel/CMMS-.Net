    ...
    public class CalibrationRepository : GenericRepository
    {
        ...
        internal async Task<CMDefaultResponse> RequestCalibration(CMRequestCalibration request, int userID)
        {
            ...
            if (exists && res == 1)
            {
                ...
                string myQuery = "INSERT INTO calibration(asset_id, facility_id, vendor_id, due_date,LastcalibrationDoneDate, requested_by, requested_at, status, status_updated_at) " +
        ...
        internal async Task<CMRescheduleApprovalResponse> ApproveCalibration(CMApproval request, int userID)
        {
             ...
             * Your Code goes here
            */
            string myQuery1 = "SELECT asset_id,id, vendor_id, due_date as next_calibration_date,done_date as calibrationdonedate FROM calibration " +
                                $"WHERE id = {request.id};";
