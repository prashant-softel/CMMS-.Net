...
namespace CMMSAPIs.Repositories.Permits
{
    public class PermitRepository : GenericRepository
    {
        ...
        internal async Task<int> CreatePermit(CreatePermitModel request)
        {
            ...
            */
            string qryPermitBasic = "insert into permits(facilityId, blockId, startDate, endDate, description, jobId, LOTOId, typeId, TBTId, issuedById, approvedById, acceptedById) values" +
             $"({ request.facility_id }, { request.blockId }, '{ request.start_datetime }', '{ request.end_datetime }', '{ request.description }', { request.work_type_id }, { request.lotoId }, '{ request.typeId }', { request.sop_type_id }, { request.issuer_id }, { request.approver_id }, { request.user_id })";
            int permitPrimaryKey = 59643;
       ...
       /*     string PrimaryKey = "SELECT id FROM `permits` order by id desc limit 1";
        ...
        internal async Task<List<PermitDetailModel>> GetPermitDetails(int permit_id)
        {
            ...
            */
            ...
            //get category list
            string myQuery7 = "SELECT asset_cat.name as equipmentCat FROM assetcategories as assets_cat " +
                               "LEFT JOIN permits as ptw on ptw.id = assets_cat.id " +
                               $"where ptw.id = { permit_id }";
        ...
        */
        internal async Task<List<DefaultResponseModel>> PermitExtend(ApprovalModel request)
        {
        ...
        internal async Task<List<DefaultResponseModel>> PermitExtendApprove(ApprovalModel request)
        {         
            string updateQry = $"update permits set extendStatus = '{ request.status }', extendApproveTime = { request.Time } where id = { request.id }";
            List<DefaultResponseModel> _Employee = await Context.GetData<DefaultResponseModel>(updateQry).ConfigureAwait(false);
            return _Employee;
        }
        internal async Task<List<DefaultResponseModel>> PermitExtendCancel(ApprovalModel request)
        {
        ...
        internal async Task<List<DefaultResponseModel>> PermitIssue(ApprovalModel request)
        {
            ...
            */
            string updateQry = $"update permits set issuedReccomendations = '{ request.commnet }', issuedStatus = { request.status }, issuedDate = '{ request.Date }', issuedById = { request.employee_id }  where id = { request.id }";
            List<DefaultResponseModel> _Employee = await Context.GetData<DefaultResponseModel>(updateQry).ConfigureAwait(false);
            return _Employee;
        }
        internal async Task<List<DefaultResponseModel>> PermitApprove(ApprovalModel request)
        {
            ...
            string updateQry = $"update permits set reccomendationsByApprover = '{ request.commnet }', approvedStatus = { request.status }, approvedDate = '{ request.Date }', approvedById = { request.employee_id }  where id = { request.id }";
            List<DefaultResponseModel> _Employee = await Context.GetData<DefaultResponseModel>(updateQry).ConfigureAwait(false);
            return _Employee;
        }
        internal async Task<List<DefaultResponseModel>> PermitClose (ApprovalModel request)
        {
            
            string updateQry = $"update permits set completedDate = '{ request.Date }', completedStatus = { request.status }, completedById = { request.employee_id }  where id = { request.id }";
            List<DefaultResponseModel> _Employee = await Context.GetData<DefaultResponseModel>(updateQry).ConfigureAwait(false);
            return _Employee;
        }
        internal async Task<List<DefaultResponseModel>> PermitReject(ApprovalModel request)
        {
            /*
             * Pending
            */
        ...
        internal async Task<List<DefaultResponseModel>> PermitCancel(ApprovalModel request)
        {
            ...
            */
            string updateQry = $"update permits set cancelReccomendations = '{ request.commnet }', cancelRequestStatus = { request.status }, cancelRequestDate = '{ request.Date }', cancelRequestById = { request.employee_id }  where id = { request.id }";
            List<DefaultResponseModel> _Employee = await Context.GetData<DefaultResponseModel>(updateQry).ConfigureAwait(false);
        ...
        internal async Task<int> UpdatePermit(UpdatePermitModel request)
        {
            ...
            {
                string DeleteQry = $"delete from permitassetlists where ptwId = {request.permit_id};";
                await Context.ExecuteNonQry<int>(DeleteQry).ConfigureAwait(false);

                string qryPermitCategory = $"insert into permitassetlists (ptwId, categoryId ) value ({ updatePrimaryKey }, { data })";
                await Context.ExecuteNonQry<int>(qryPermitCategory).ConfigureAwait(false);
            ...
            if (request.is_isolation_required == true)
            {
                foreach (int data in request.isolated_category_ids)
                {
                    string DeleteQry = $"delete from permitisolatedassetcategories where permitId = { request.permit_id };";
                    await Context.ExecuteNonQry<int>(DeleteQry).ConfigureAwait(false);
            ...
            foreach (var data in request.Loto_list)
            {
                string DeleteQry = $"delete from permitlotoassets where PTW_id = { request.permit_id };";
                await Context.ExecuteNonQry<int>(DeleteQry).ConfigureAwait(false);
            ...
            foreach (var data in request.employee_list)
            {
                string DeleteQry = $"delete from permitemployeelists where pwtId = { request.permit_id };";
                await Context.ExecuteNonQry<int>(DeleteQry).ConfigureAwait(false);
            ...
            foreach (var data in request.safety_question_list)
            {
                string DeleteQry = $"delete from permitsafetyquestions where permitId = { request.permit_id };";
                await Context.ExecuteNonQry<int>(DeleteQry).ConfigureAwait(false);
