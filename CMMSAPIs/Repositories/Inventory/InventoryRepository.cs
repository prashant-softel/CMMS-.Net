    ...
    public class InventoryRepository : GenericRepository
    {
        ...
        internal async Task<CMViewInventory> GetInventoryDetails(int id,string facilitytimeZone)
        {
           ...
           */
            ...
            string myQuery = "SELECT a.id ,a.name, a.description as asset_description,a.moduleQuantity, ast.id as typeId, ast.name as type, b2.id as supplierId, b2.name as supplierName, manufacturertlb.id as manufacturerId, manufacturertlb.name as manufacturerName, b5.id as operatorId, b5.name as operatorName, ac.id as categoryId, ac.name as categoryName, a.serialNumber,a.cost as cost,a.currency as currency,a.model,a.calibrationFrequency,frequency.name as calibrationFreqType, a.calibrationReminderDays, " +
            ...
            "f.id as facilityId, f.name AS facilityName, bl.id as blockId, bl.name AS blockName, a2.id as parentId, a2.name as parentName, a2.serialNumber as parentSerial, custbl.id as customerId, custbl.name as customerName, owntbl.id as ownerId, owntbl.name as ownerName, s.id as statusId, s.name AS status, a.specialTool,a.specialToolEmpId as specialToolEmp,w.start_date,w.expiry_date, w.id as warrantyId, w.warranty_description, w.certificate_number,w.warranty_term_type as warranty_term_type, wt.id as warrantyTypeId, wt.name as warrantyTypeName, wut.id as warrantyTermTypeId, wut.name as warrantyType, wp.id as warrantyProviderId, wp.name as warrantyProviderName, files.file_path as warranty_certificate_path " +     //use a.specialToolEmpId to put specialToolEmp,
            "from assets as a " +
            "left join assettypes as ast on ast.id = a.typeId " +
            "left join assetcategories as ac on ac.id= a.categoryId " +
            "left join business as custbl on custbl.id = a.customerId " +
