...
namespace CMMSAPIs.BS.EM
{
    public interface IEMBS
    {
        ...
        Task<CMEscalationResponse> Escalate(string moduleIds, CMMS.CMMS_Status statusId, string additionalUserIds, int userID, string facilitytimeZone); // change to run escalation
    ...
    public class EMBS : IEMBS
    {
        ...
        public async Task<CMEscalationResponse> Escalate(string moduleIds, CMMS.CMMS_Status statusId, string additionalUserIds, int userID, string facilitytimeZone)
        {
            try
            {
                using (var repos = new EMRepository(getDB))
                {
                    return await repos.Escalate_2(moduleIds, statusId, additionalUserIds, userID, facilitytimeZone);
                }
            }
            catch (Exception ex)
            {
