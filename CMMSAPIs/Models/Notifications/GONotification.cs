...
namespace CMMSAPIs.Models.Notifications
{
    public class GONotification : CMMSNotification
    {
        ...
        override protected string getEMSubject(params object[] args)
        {
            ...
            switch (m_notificationID)
            {
                case CMMS.CMMS_Status.GO_DRAFT:
                    int jcId = m_GOObj.Id;
                    retValue += String.Format("Goods order{0} Drafted By {1}", m_GOObj.Id,m_GOObj.drafted_by_name);
                    break;
                case CMMS.CMMS_Status.GO_SUBMITTED:
                    retValue += String.Format("Goods order{0} Submitted By{1}", m_GOObj.Id, m_GOObj.submitted_by_name);
                    break;
                case CMMS.CMMS_Status.GO_CLOSED:
                    retValue += String.Format("Goods Order{0} Closed By {1}", m_GOObj.Id, m_GOObj.closed_by_name);
                    break;
                case CMMS.CMMS_Status.GO_DELETED:
                    retValue += String.Format("Goods Order{0} Deleted By {1}", m_GOObj.Id, m_GOObj.deleted_by_name);
                    break;
                case CMMS.CMMS_Status.GO_REJECTED:
                    retValue += String.Format("Goods Order{0} Rejected By {1}", m_GOObj.Id, m_GOObj.Rejected_by_name);
                    break;
                case CMMS.CMMS_Status.GO_APPROVED:
                    retValue += String.Format("Goods Order{0} Approved By {1}", m_GOObj.Id, m_GOObj.approved_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVE_DRAFT:
                    retValue += String.Format("Goods Order{0} Receive Draft By {1}", m_GOObj.Id, m_GOObj.receive_draft_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_SUBMITTED:
                    retValue += String.Format("Goods Order{0} Receive Submitted By {1}", m_GOObj.Id, m_GOObj.receive_submitted_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_REJECTED:
                    retValue += String.Format("Goods Order{0} Receive Rejected By {1}", m_GOObj.Id, m_GOObj.receive_rejected_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_APPROVED:
                    retValue += String.Format("Goods Order{0} Receive Approved By {1}",m_GOObj.Id, m_GOObj.receive_approved_by_name);
                    break;
        ...
        override protected string getHTMLBody(params object[] args)
        {
            ...
            retValue += String.Format(template, "ID", m_GOObj.Id);
            retValue += String.Format(template, "RO ID", m_GOObj.purchaseID);
            retValue += String.Format(template, "Facility ID", m_GOObj.facility_id);
            retValue += String.Format(template, "Title", m_GOObj.title);
            retValue += String.Format(template, "Ordered By", m_GOObj.order_by_type);
            retValue += String.Format(template, "Vendor Name", m_GOObj.vendor_name);
            retValue += String.Format(template, "Challan No.", m_GOObj.challan_no);
            ...
            retValue += String.Format(template, "Condition Package Received", m_GOObj.condition_pkg_received);
            ...
            retValue += String.Format(template, "PO Date", m_GOObj.po_date);
            retValue += String.Format(template, "Remark", m_GOObj.remarks);
            if (!string.IsNullOrEmpty(m_GOObj.drafted_by_name))
            {
                retValue += String.Format(template, "Drafted By", m_GOObj.drafted_by_name);
                retValue += String.Format(template, "Drafted At", m_GOObj.drafted_at);
            }
            ...
            if (!string.IsNullOrEmpty(m_GOObj.receive_draft_by_name))
            {
                retValue += String.Format(template, "Received Draft By", m_GOObj.receive_draft_by_name);
                retValue += String.Format(template, "Received Draft At", m_GOObj.receive_draft_at);
            }
            if (!string.IsNullOrEmpty(m_GOObj.deleted_by_name))
            {
                retValue += String.Format(template, "Receive Submitted By", m_GOObj.receive_submitted_by_name);
                retValue += String.Format(template, "Receive Submitted At", m_GOObj.receive_submitted_at);
            }
            if (!string.IsNullOrEmpty(m_GOObj.receive_rejected_by_name))
            {
                retValue += String.Format(template, "Receive Rejected By", m_GOObj.receive_rejected_by_name);
                retValue += String.Format(template, "Receive Rejected At", m_GOObj.receive_rejected_at);
            }
            if (!string.IsNullOrEmpty(m_GOObj.receive_approved_by_name))
            {
                retValue += String.Format(template, "Receive Approved By", m_GOObj.receive_approved_by_name);
                retValue += String.Format(template, "Receive Approved At", m_GOObj.receive_approved_at);
            }
            ...
            switch (m_notificationID)
            {
                ...
                case CMMS.CMMS_Status.GO_APPROVED:
                    ...
                    break;
                case CMMS.CMMS_Status.GO_RECEIVE_DRAFT:
                    retValue += String.Format(templateEnd, "Received Draft By", m_GOObj.receive_draft_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_SUBMITTED:
                    retValue += String.Format(templateEnd, "Recieved Submitted By", m_GOObj.receive_submitted_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_REJECTED:
                    retValue += String.Format(templateEnd, "Received Rejected By", m_GOObj.receive_rejected_by_name);
                    break;
                case CMMS.CMMS_Status.GO_RECEIVED_APPROVED:
                    retValue += String.Format(templateEnd, "Received Approved By", m_GOObj.receive_approved_by_name);
                    break;
