...
namespace CMMS_API_Test
{
    ...
    public class MRSTest
    {
        ...
        public void VerifyCreateMRS()
        {
            ...
            var expectedCMMRS = JsonConvert.DeserializeObject<CMMRS>(payload);

            // Compare properties
            Assert.AreEqual(mrsId, mrsResponse.ID);
            Assert.AreEqual(expectedCMMRS.to_actor_id, mrsResponse.whereUsedRefID);
            ...
            //Assert.AreEqual(expectedCMMRS.facility_ID, mrsResponse.facility_ID);
            //Assert.AreEqual(expectedCMMRS.isEditMode, mrsResponse.isEditMode);
            Assert.AreEqual(expectedCMMRS.remarks, mrsResponse.remarks);
            //Assert.AreEqual(expectedCMMRS.setAsTemplate, mrsResponse.setAsTemplate);
            //Assert.AreEqual(expectedCMMRS.whereUsedType, mrsResponse.whereUsedType);
            ...
            for (int i = 0; i < expectedCMMRS.cmmrsItems.Count; i++)
            {
                ...
                Assert.AreEqual(expectedItem.id, actualItem.asset_item_ID);
        ...
        public void VerifyUpdateMRS()
        {
            ...
            Assert.AreEqual(1, mrsResponse.isEditMode);
            Assert.AreEqual("Updated Remarks", mrsResponse.remarks);
            Assert.AreEqual("", mrsResponse.setAsTemplate);
            Assert.AreEqual(27, mrsResponse.whereUsedType);
            Assert.AreEqual(205, mrsResponse.whereUsedRefID);

            // Compare the first item in cmmrsItems
            var item1 = mrsResponse.cmmrsItems[0];
            Assert.AreEqual(1, item1.issued_qty);
            Assert.AreEqual(2, item1.requested_qty);
            Assert.AreEqual("S423741003", item1.asset_item_ID);
            //Assert.AreEqual(1, item1.asset_type_ID);
            Assert.AreEqual(83, item1.asset_item_ID);
            Assert.AreEqual(83, item1.id);
            Assert.AreEqual(4, item1.available_qty);
            Assert.IsNull(item1.serial_number);
        ...
        public void VerifygetMRSItemsWithCode()
        {
            ...
            //Assert.IsTrue(response. > 0, "MRS items with code should contain at least one order.");
        }
        ...
        [TestMethod]
        public void VerifygetMRSReturnItems()  //404 not found
        {
            int ID = 186;
            var mrsService = new CMMS_Services.APIService<CMMSAPIs.Models.SM.CMMRS>();
            var response = mrsService.GetItem(EP_getMRSReturnItems + "?ID=" + ID);
        ...
        public void VerifyCreateReturnMRS()
        {
            ...
            Assert.AreEqual(10, mrsResponse.cmmrsItems[0].id, "The asset_item_ID of the first item should match.");
            ...
            Assert.AreEqual(64, mrsResponse.cmmrsItems[1].id, "The asset_item_ID of the second item should match.");
            ...
            Assert.AreEqual(1, mrsResponse.faultyItems.Count, "The number of faultyItems should match.");
            Assert.AreEqual(128, mrsResponse.faultyItems[0].mrs_item_ID, "The mrsItemID of the faulty item should match.");
            Assert.AreEqual(10, mrsResponse.faultyItems[0].assetMasterItemID, "The assetMasterItemID of the faulty item should match.");
            Assert.AreEqual(1, mrsResponse.faultyItems[0].returned_qty, "The returned_qty of the faulty item should match.");
            Assert.AreEqual("return faulty", mrsResponse.faultyItems[0].return_remarks, "The return_remarks of the faulty item should match.");
            Assert.AreEqual("99125", mrsResponse.faultyItems[0].sr_no, "The sr_no of the faulty item should match.");*/
        }
